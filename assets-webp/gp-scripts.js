...
(function () {
  ...
  // Mark above-the-fold elements as already revealed BEFORE arming the transitions
  els.forEach(el => {
    const r = el.getBoundingClientRect();
    if (r.top < window.innerHeight * 0.95 && r.bottom > 0) el.classList.add('in');
  });
  // Arm reveal transitions on next frame so above-the-fold items skip the animation
  requestAnimationFrame(() => {
